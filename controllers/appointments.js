...
exports.getAppointment = async (req , res , next)=> {
    try{
        ...
        if(!appointment){
            return res.status(404).json({success : false , message: `No appointment with the id of ${hid}`});
        }
...
exports.addAppointment = async (req , res , next)=> {
    try{
        ...
        if(!hospital){
            return res.status(404).json({success:false ,massage : `No hospital with the id of ${hid}`})
        }
...
exports.updateAppointment = async (req , res , next)=> {
    try{
        ...
        if(appointment.user.toString() !== user.id && user.role !== "admin" ){
            return res.status(404).json({success:false ,massage : `User ${user.id} is not authorized to update this appointment`})
        }

        appointment = await Appointment.findByIdAndUpdate(apptid , req.body , { new:true, runValidators:true});
